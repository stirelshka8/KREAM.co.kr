...
logging.basicConfig(level=logging.INFO, filename=f"logger.log", filemode="a",
...
def save_file():
    ...
    writer = pd.ExcelWriter(config['CONFIG']['save'], engine='xlsxwriter')
    df.to_excel(writer, sheet_name=search, index=True)
    writer.save()


def update_files():
    threading.Timer(temer, update_files).start()
    logging.info(f"Последнее обновление {datetime.datetime.now()}")
    save_file()
